...
protected:
	...
	virtual Node<Key, Value>* insertHelper(Node<Key, Value>* node, const std::pair<Key, Value>& keyValuePair);
...
template<typename Key, typename Value>
Node<Key, Value>* BinarySearchTree<Key, Value>::insertHelper(Node<Key, Value>* node, const std::pair<Key, Value>& keyValuePair)
{
	if(node == NULL)
	{
